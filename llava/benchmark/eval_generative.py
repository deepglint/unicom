...
def create_model(rank, model_path):
    ...
    tokenizer, model, _, _ = load_pretrained_model(model_path, None, model_name, device_map=rank, attn_implementation='sdpa', torch_dtype='auto')
    model = model.to(rank)
...
def run(dataloader: DataLoader, model, tokenizer, eval_args):
    ...
    for _, batch in enumerate(pbar):
        ...
        batch = bmk.prepare_inputs(batch, dtype=torch.float32, device=rank)
