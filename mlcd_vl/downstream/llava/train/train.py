...
from llava.constants import IGNORE_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, IMAGE_TOKEN_INDEX, DEFAULT_SEG_TOKEN
...
class ModelArguments:
    ...
    sam_path : Optional[str] = field(default=None)
...
class LazySupervisedDataset(Dataset):
    ...
    def _process_sam_image(self, image_file: Union[bytes, str]):
        try:
            if isinstance(image_file, bytes):
                image = Image.open(BytesIO(image_file)).convert('RGB')
            elif isinstance(image_file, str):
        ...
        image_sam_resize = image_np_resize.shape[:2]
        grounding_enc_img = self.grounding_enc_processor(torch.from_numpy(image_np_resize).permute(2, 0, 1).contiguous())
        return grounding_enc_img, image_sam_resize, original_size
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "images" in sources[0] and len(sources[0]['images']) < 9:
            ...
            if type(image_file) is list:
                ...
                sam_image = [self._process_sam_image(f) for f in image_file]
                ...
                sam_image = [self._process_sam_image(image_file)]
        ...
        if len(sam_image)==0:
            masks_list = [torch.zeros(0, mask_h, mask_w, dtype=torch.uint8)]
            sam_image = [(torch.zeros(3, 1024, 1024), (1024,1024), (336, 366))]
        else:
            if "segmentation" in self.list_data_dict[i]:
                mask_h, mask_w = sam_image[0][2][0], sam_image[0][2][1]
                masks_list = []
                for rle in self.list_data_dict[i]["segmentation"]:
                    m = mask_util.decode(rle)
                    masks_list.append(torch.from_numpy(m).unsqueeze(0))
            else:
                masks_list = [torch.zeros(0, mask_h, mask_w, dtype=torch.uint8)]
        ...
        grounding_enc_img_list = [sam_i[0] for sam_i in sam_image]
        image_sam_resize_list = [sam_i[1] for sam_i in sam_image]
        data_dict["grounding_enc_imgs"] = torch.stack(grounding_enc_img_list, dim=0)
        data_dict["image_sam_resize_list"] = image_sam_resize_list
        data_dict["original_size"] = (mask_h, mask_w)
        data_dict["masks_list"] = torch.cat(masks_list, dim=0)
        return data_dict
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "grounding_enc_imgs" in instances[0]:
            batch["grounding_enc_imgs"] = [instance["grounding_enc_imgs"] for instance in instances]
            batch["image_sam_resizes"] = [instance["image_sam_resize_list"] for instance in instances]
            batch["original_sizes"] = [instance["original_size"] for instance in instances]
            batch["masks_list"] = [instance["masks_list"] for instance in instances]
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        ...
        elif "glint" in model_args.model_name_or_path.lower():
            model = LlavaQwenForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                attn_implementation=training_args.attn_implementation,
...
def train(attn_implementation=None):
    ...
    elif "qwen" in model_args.model_name_or_path.lower() or "glint" in model_args.model_name_or_path.lower():
    ...
    if model_args.vision_tower is not None:
        ...
        for key in vars(model_args):
            if key.startswith('sam_') and getattr(model_args, key) is not None:
                setattr(model.config, key, getattr(model_args, key)) 
        if getattr(model_args, 'sam_path', None) is not None and model_args.sam_path !="":
            model.get_model().sam.to(dtype=compute_dtype, device=training_args.device)
            model.get_model().text2sam_projection.to(dtype=compute_dtype, device=training_args.device)
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            if getattr(model_args, 'sam_path', None) is not None and model_args.sam_path !="":
                # model.requires_grad_(False)
                for p in model.get_model().sam.parameters():
                    p.requires_grad = True
                for p in model.get_model().text2sam_projection.parameters():
                    p.requires_grad = True
                for p in model.get_model().sam.image_encoder.parameters():
                    p.requires_grad = False
